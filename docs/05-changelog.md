...
## v4.2.0 — 2026-08-30
- [추가] **경량 오프닝 파티클 (Hero Canvas 2D)** — Hero 배경에 뉴런 노드·연결선 파티클 애니메이션
  - 음성 파형 + 뉴런 네트워크 모션을 은은하게 표현 — 라이트 브랜드 유지 (다크모드 전환 없음)
  - Canvas 2D 자체 구현 — tsParticles/Three.js 등 외부 라이브러리 0 (의존성 0 원칙)
  - BAOS-MOTION-003: `next/dynamic(ssr:false)` 동적 import → 전용 청크 4KB로 분리, LCP 영향 0
  - BAOS-MOTION-001: `prefers-reduced-motion` 사용자 → 정적 프레임 1장만 렌더, 면적 기반 파티클 밀도(모바일 자동 감소)
- [검증] BQAE 동작 검증 **13/13 통과** (신규: 캔버스 존재·실제 드로잉·동적 로딩·61fps)
- [검증] 번들 JS gzip 199.1KB (v4.1.0 대비 +2.2KB, 200KB 기준 내) — BAOS-MOTION-002 기록
- [검증] BAOS-HTML-001/002/004 구조 검증 통과 (div 303=303, section 7=7)
